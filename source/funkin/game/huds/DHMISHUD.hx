...
class DHMISHUD extends BaseHUD
{
	...
	override function init()
	{
		...
		healthBarFG = new FlxSprite(-81, -88).loadGraphic(Paths.image('ui/dhmis/healthbarFG'));
		healthBarFG.antialiasing = ClientPrefs.globalAntialiasing;
		healthBarFG.scale.set(1.01, 1);
		...
		scoreTxt.setFormat(Paths.font("Neuzeit.otf"), 20, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	override function popUpScore(ratingImage:String, combo:Int)
	{
		...
		final posY = FlxG.height * (ClientPrefs.downScroll ? 0.84 : 0.09);

		if (showRating)
		{
			FlxTween.cancelTweensOf(ratingGraphic, ['scale.x', 'scale.y', 'alpha']);
			ratingGraphic.alpha = 1;
			ratingGraphic.loadGraphic(Paths.image('ui/dhmis/ratings/' + ratingImage));
		...
		if (showRatingNum)
		{
			...
			for (i in seperatedScore)
			{
				...
				numScore.loadGraphic(Paths.image('ui/dhmis/ratings/' + Std.int(i)));
				numScore.alpha = 1;
				numScore.screenCenter();
				numScore.x = posX + (25 * daLoop) - 37;
				numScore.y = posY + (ClientPrefs.downScroll ? -40 : 70);
