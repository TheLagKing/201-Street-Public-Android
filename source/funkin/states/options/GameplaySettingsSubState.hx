...
class GameplaySettingsSubState extends BaseOptionsMenu
{
	public function new()
	{
		...
		var option:Option = new Option('Keyboard Enabled',
			'Check this if you want to play with a keyboard on the Android Port',
			'keyboardEnabled',
			'bool',
			false);
		addOption(option);
...
*/
		...
		var option:Option = new Option('Ghost Tapping', "Prevents misses from pressing keys while there are no notes able to be hit", 'ghostTapping', 'bool',
			true);
		addOption(option);

		var option:Option = new Option('Disable Reset Button', "Prevents the reset button in gameplay", 'noReset', 'bool', false);
		addOption(option);

		var option:Option = new Option('Hitsound Volume', 'Hitting notes play a sound', 'hitsoundVolume', 'percent', 0);
		addOption(option);
		...
		var option:Option = new Option('Use Epic Ratings', // Name
			'Adds Epic Ratings as a bonus judgement above sick (does not affect accuracy, only your score).', // Description
		...
		var option:Option = new Option('Dev Mode', "Enables hot-reloading with F5 and F6, not recommended unless you are debugging", 'inDevMode', 'bool', true);
		addOption(option);
