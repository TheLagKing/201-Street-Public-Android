...
class ControlsSubState extends MusicBeatSubstate
{
	...
	public function new()
	{
		...
		for (i in 0...optionShit.length)
		{
			...
			var optionText:ControlText = new ControlText(0, (10 * i), optionShit[i][0]);
	...
	private function addBindTexts(optionText:ControlText, num:Int)
	{
		...
		var text1 = new AttachedControlText(InputFormatter.getKeyName(keys[0]), 400, 0);
		text1.setPosition(optionText.x + 400, optionText.y);
		text1.sprTracker = optionText;
		grpInputs.push(text1);
		add(text1);

		var text2 = new AttachedControlText(InputFormatter.getKeyName(keys[1]), 650, 0);
