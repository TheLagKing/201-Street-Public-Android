...
class WeekEditorState extends MusicBeatState
{
	...
	{
		var tabs = [
			{name: 'Week', label: 'Week'}, {name: 'Other', label: 'Other'},];
	...
	function reloadBG()
	{
		...
		{
			if (FunkinAssets.exists(Paths.getPath('images/menubackgrounds/menu_$assetName.png', IMAGE, null, true)))
			{
	...
	function reloadWeekThing()
	{
		...
		if (assetName != null && assetName.length > 0)
		{
			if (FunkinAssets.exists(Paths.getPath('images/storymenu/$assetName.png', IMAGE, null, true)))
			{
...
class WeekEditorFreeplayState extends MusicBeatState
{
	...
	override function create()
	{
		bg = new FlxSprite().makeGraphic(1280, 720, FlxColor.WHITE);
		add(bg);
	...
	function changeSelection(change:Int = 0)
	{
		...
		if (curSelected < 0) curSelected = weekFile.songs.length - 1;
		if (curSelected >= weekFile.songs.length) curSelected = 0;
