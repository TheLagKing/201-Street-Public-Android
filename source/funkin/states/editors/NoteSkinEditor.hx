...
class NoteSkinEditor extends MusicBeatState
{
	...
	function addTexturesUI()
	{
		...
		skinDropDown = new FlxUIDropDownMenuEx(inEngineColoringCheck.x + inEngineColoringCheck.width + 10, 90,
			FlxUIDropDownMenu.makeStrIdLabelArray([''], true), (skin) -> {
	...
	function addAnimationsUI()
	{
		...
		loopingCheck = new FlxUICheckBox(animationNameInputText.x + animationNameInputText.width + 20, animationNameInputText.y, null, null,
			"Does animation loop?");
		...
		if (id == FlxUINumericStepper.CHANGE_EVENT && (sender is FlxUINumericStepper))
		{
			...
			switch (wname)
			{
				...
				case 'keys':
					...
					if (handler.data.noteAnimations.length < keys)
						handler.data.noteAnimations.push(NoteSkinHelper.fallbackNote(handler.data.noteAnimations.length));
					if (handler.data.noteSplashAnimations.length < keys)
						handler.data.noteSplashAnimations.push(NoteSkinHelper.fallbackSplash(handler.data.noteSplashAnimations.length));
