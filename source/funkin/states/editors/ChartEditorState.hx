...
class ChartEditorState extends MusicBeatState
{
	...
	var eventStuff:Array<Dynamic> = [
		...
		[
			'Camera Follow Pos',
			'Value 1: X,Y\n Value 2: Set to "snap" if you want it to snap to the pos, otherwise leave blank'
		],
		[
			'Set Character Cam Pos',
			"Value 1: X,Y\n Value 2: Character to change cam pos of (Dad, BF, GF)"
		],
		[
			'Set Cam Zoom',
	...
	**/
	...
	override function create()
	{
		...
		bg = new FlxSprite().makeGraphic(1280, 720, 0xFF222222);
		bg.scrollFactor.set();
		add(bg);

		gradient = FlxGradient.createGradientFlxSprite(FlxG.width, FlxG.height * 8,
			[0x0, ClientPrefs.editorGradColors[0], ClientPrefs.editorGradColors[1], 0x0]);
	...
	function addSongUI():Void
	{
		...
		var stepperStrums:FlxUINumericStepper = new FlxUINumericStepper(stepperBPM.x + (stepperBPM.width * 2), 70, 1, 2, 1, 8, 0, 1,
			new FlxUIInputTextEx(0, 0, 25));
		stepperStrums.value = _song.lanes;
		stepperStrums.name = 'song_strums';
		blockPressWhileTypingOnStepper.push(stepperStrums);

		var stepperKeys:FlxUINumericStepper = new FlxUINumericStepper(stepperBPM.x + (stepperBPM.width * 2), 100, 1, 2, 4, 9, 0, 1,
			new FlxUIInputTextEx(0, 0, 25));
		...
		var gfVersionDropDown = new FlxUIDropDownMenuEx(player1DropDown.x, player1DropDown.y + 40, FlxUIDropDownMenu.makeStrIdLabelArray(characters, true),
			function(character:String) {
		...
		var player2DropDown = new FlxUIDropDownMenuEx(player1DropDown.x, gfVersionDropDown.y + 40, FlxUIDropDownMenu.makeStrIdLabelArray(characters, true),
			function(character:String) {
		...
		stageDropDown = new FlxUIDropDownMenuEx(player1DropDown.x + 140, player1DropDown.y, FlxUIDropDownMenu.makeStrIdLabelArray(stages, true),
			function(character:String) {
	...
	function addVisualsUI():Void
	{
		...
		var gradient1colors = new FlxUIInputTextEx(10, 30, 150,
			'${ClientPrefs.editorGradColors[0].red}, ${ClientPrefs.editorGradColors[0].green}, ${ClientPrefs.editorGradColors[0].blue}', 8);
		var gradient2colors = new FlxUIInputTextEx(10, 50, 150,
			'${ClientPrefs.editorGradColors[1].red}, ${ClientPrefs.editorGradColors[1].green}, ${ClientPrefs.editorGradColors[1].blue}', 8);
		...
		var boxTxtColors1 = new FlxUIInputTextEx(10, 115, 150,
			'${ClientPrefs.editorBoxColors[0].red}, ${ClientPrefs.editorBoxColors[0].green}, ${ClientPrefs.editorBoxColors[0].blue}', 8);
		var boxTxtColors2 = new FlxUIInputTextEx(10, 135, 150,
			'${ClientPrefs.editorBoxColors[1].red}, ${ClientPrefs.editorBoxColors[1].green}, ${ClientPrefs.editorBoxColors[1].blue}', 8);
		...
		var uiBoxTxt = new FlxUIInputTextEx(10, 175, 150,
			'${ClientPrefs.editorUIColor.red}, ${ClientPrefs.editorUIColor.green}, ${ClientPrefs.editorUIColor.blue}', 8);
	...
	function addSectionUI():Void
	{
		...
		stepperBeats = new FlxUINumericStepper(10, 100, 1, 4, 1, 16, 2, 1, new FlxUIInputTextEx(0, 0, 25));
	...
	function reloadGridLayer()
	{
		...
		gridBG = FlxGridOverlay.create(GRID_SIZE, GRID_SIZE, GRID_SIZE * ((_song.keys * _song.lanes) + 1),
			Std.int(GRID_SIZE * getSectionBeats() * 4 * zoomList[curZoom]), true, ClientPrefs.editorBoxColors[0], ClientPrefs.editorBoxColors[1]);
		#if desktop
		...
		{
			prevGridBG = FlxGridOverlay.create(GRID_SIZE, GRID_SIZE, GRID_SIZE * ((_song.keys * _song.lanes) + 1),
				Std.int(GRID_SIZE * getSectionBeats(curSec - 1) * 4 * zoomList[curZoom]), true,
		...
		{
			nextGridBG = FlxGridOverlay.create(GRID_SIZE, GRID_SIZE, GRID_SIZE * ((_song.keys * _song.lanes) + 1),
				Std.int(GRID_SIZE * getSectionBeats(curSec + 1) * 4 * zoomList[curZoom]), true,
				FlxColor.fromRGB(ClientPrefs.editorBoxColors[0].red - 40, ClientPrefs.editorBoxColors[0].green - 40, ClientPrefs.editorBoxColors[0].blue - 40),
				FlxColor.fromRGB(ClientPrefs.editorBoxColors[1].red - 40, ClientPrefs.editorBoxColors[1].green - 40, ClientPrefs.editorBoxColors[1].blue
					- 40));
	...
	function loadHealthIconFromCharacter(char:String)
	{
		...
		#end
		{
			path = Paths.getCorePath('characters/' + Character.DEFAULT_CHARACTER +
