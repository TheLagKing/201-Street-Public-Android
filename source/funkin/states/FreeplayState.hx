...
class FreeplayState extends MusicBeatState
{
	...
	override function create()
	{
		...
		if (WeekData.weeksList.length == 0)
		{
			...
			FlxG.switchState(() -> new FallbackState('cannot load Freeplay as there are no weeks loaded.', () -> FlxG.switchState(TitleState201.new)));
			return;
		}
		...
		if (isHardcodedState())
		{
			bg = new FlxSprite().makeGraphic(1280, 720, 0xFFDBDBDB);
			add(bg);
			bg.screenCenter();
