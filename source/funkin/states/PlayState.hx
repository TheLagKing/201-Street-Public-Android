...
class PlayState extends MusicBeatState
{
	...
	**/
	...
	public var ratingsData:Array<Rating> = [new Rating('sick'), new Rating('good'), new Rating('bad'), new Rating('shit')];
	...
	override public function create():Void
	{
		...
		playHUD = new funkin.game.huds.Two01HUD(this);
			...
			for (songNotes in section.sectionNotes)
			{
				...
				for (susNote in 0...flooredSusLength + 1)
				{
					...
					var sustainNote:Note = new Note(daStrumTime
						...
						+ (Conductor.stepCrotchet / FlxMath.roundDecimal(songSpeed, 2)), daNoteData, oldNote,
						true, false, gottaHitNote ? 0 : 1);
	...
	public inline function getTimeFromSV(time:Float,
			event:SpeedEvent):Float return event.position + (modManager.getBaseVisPosD(time - event.songTime, 1) * event.speed);
	...
	override public function update(elapsed:Float):Void
	{
		...
		if (generatedMusic)
		{
			...
			notes.forEachAlive(function(daNote:Note) {
				...
				final pos = modManager.getPos(daNote.strumTime, visPos, daNote.strumTime - Conductor.songPosition, curDecBeat, daNote.noteData, daNote.lane,
					daNote, null, daNote.vec3Cache);
				...
				if (Conductor.songPosition > noteKillOffset + daNote.strumTime)
				{
					...
					if (daNote.playField != null && daNote.playField.playerControls && !daNote.playField.autoPlayed && !daNote.ignoreNote && !endingSong
						&& (daNote.tooLate || !daNote.wasGoodHit)) if (field.playerControls && !field.autoPlayed) field.noteMissCallback.dispatch(daNote,
							field);
				}
	...
	public function updateScoreBar(miss:Bool = false):Void
	{
		if (scripts.call('onUpdateScore',
			[miss]) != ScriptConstants.Function_Stop) callHUDFunc(hud -> hud.onUpdateScore(songScore,
				funkin.utils.MathUtil.floorDecimal(ratingPercent * 100, 2), songMisses, miss));
	}
		...
		if (char != null)
		{
			if (!note.hitCausesMiss)
			{
				if (!note.noAnimation)
				{
					...
					if (ClientPrefs.jumpGhosts && char.ghostsEnabled && !note.isSustainNote && chord != null && chord.length > 1
	...
	override function stepHit()
	{
		...
		if (Math.abs(FlxG.sound.music.time - (Conductor.songPosition - Conductor.offset)) > maxToleratedOffset
			|| (SONG.needsVoices
				&& vocals.getDesyncDifference(Math.abs(Conductor.songPosition - Conductor.offset)) > maxToleratedOffset)) resyncVocals();
	...
	override function beatHit()
	{
		...
		if (camZooming && curBeat % beatsPerZoom == 0)
		{
